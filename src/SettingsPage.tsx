...
export default function SettingsPage({settings,health,onSaved,onError,onNotice,onConnectionChanged}:{onConnectionChanged:()=>Promise<unknown>;settings:Settings;health:Health;onSaved:(s:Settings)=>void;onError:(e:string)=>void;onNotice:(e:string)=>void}){
  ...
  return <section><div className="ss-heading"><div><h1>每天，沿着好奇出发。</h1><p>定好方向，让日常制作沿着熟悉的路线进行。</p></div><span className={`ss-badge ${settings.schedule_enabled?'ss-green':''}`}><Clock3/>{settings.schedule_enabled?'每日计划已启用':'每日计划未启用'}</span></div>
    <div className="configuration-grid"><ModelSettings onChanged={onConnectionChanged}/><section className="connection-card"><h2>先配置网址，再收集灵感。</h2><p className="connection-description">采集配置在“每日选题”中。输入网址后可直接保存并执行，结果在同一页查看。</p><a className="ss-btn" href="#topics">前往配置采集数据 →</a></section></div>
