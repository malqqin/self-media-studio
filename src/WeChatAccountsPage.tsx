...
export default function WeChatAccountsPage({onError,onNotice}:{onError:(s:string)=>void;onNotice:(s:string)=>void}){
  ...
  return <section>
    <div className="platform-heading"><div><h1>发布账号</h1><p>统一管理公众号连接，任务中只需选择账号与交付方式。</p></div><button className="ss-btn" disabled={!!busy} onClick={()=>choose('')}><Plus/>添加公众号</button></div>
