...
export function ArticleWorkspace({article,illustration,busy,dirty,onDirty,onAction,onRefresh,onError,onNotice}:{article:Article;illustration?:IllustrationSettings;busy:string;dirty:boolean;onDirty:(v:boolean)=>void;onAction:(action:string,extra?:Record<string,unknown>)=>Promise<unknown>;onRefresh:()=>Promise<void>;onError:(s:string)=>void;onNotice:(s:string)=>void}){
  ...
  const blocked=!!busy||saving||active(article)||Object.values(pictureBusy).some(Boolean);
  ...
  const copy=async()=>{try{const response=await fetch(`/api/articles/${article.id}/export?format=html&wechat=true&version=${article.version}`);if(!response.ok)throw new Error('文章已更新或正在处理，请刷新后复制。');const value=await response.text();if(!navigator.clipboard?.write)throw new Error('当前浏览器不支持复制排版，请下载 HTML。');const plain=doc?[doc.title,doc.summary,doc.opening,...doc.sections.flatMap(s=>[s.heading,...s.paragraphs]),doc.closing].filter(Boolean).join('\n\n'):'';await navigator.clipboard.write([new ClipboardItem({'text/html':new Blob([value],{type:'text/html'}),'text/plain':new Blob([plain],{type:'text/plain'})})]);onNotice('已复制排版文字。粘贴到公众号后检查排版，图片请另行上传。');}catch(e){onError((e as Error).message);}};
  ...
  const imageCard=(label:string,id:string,caption:string,locked:boolean,hint:string,onChange:(id:string,caption:string,locked:boolean)=>void)=><ArticleImagePicker label={label} value={id} caption={caption} locked={locked} hint={hint} settings={pictureSettings} assets={assets} disabled={blocked} onChange={onChange} onAssets={a=>setAssets(v=>[a,...v.filter(x=>x.id!==a.id)])} onBusy={v=>{setPictureBusy(old=>old[label]===v?old:{...old,[label]:v});if(v)mark(true);}} onError={onError} onNotice={onNotice}/>;
  return <div className="article-workspace"><div className="article-workspace-heading"><div><span className="fn-label">{article.profile.name} · {article.mode==='original'?'原创构思':'资料参考'} · v{article.version}</span><h2>{doc?.title||outline?.title||'为文章寻找一个角度'}</h2></div><div className="article-heading-actions"><span className="ss-badge">{dirty?'有未保存修改':labels[article.status]}</span><ArticleVersions article={article} blocked={blocked||dirty} onRestore={version=>onAction('restore',{target_version:version})}/></div></div>
