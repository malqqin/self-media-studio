...
export default function ModelsPage({onError,onNotice}:{onError:(s:string)=>void;onNotice:(s:string)=>void}){
  ...
  return <section className="models-page"><div className="platform-heading"><div><h1>我的模型</h1><p>连接你喜欢的模型，让文字与画面各有所长。</p></div><button className="ss-btn ss-primary" onClick={()=>choose(blank)}><Plus/>添加模型</button></div>
