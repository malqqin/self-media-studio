...
import {AuthGate,AccountButton,useAuth} from './Auth';
...
const route=()=>{const value=location.hash.slice(1);return pages.some(p=>p.id===value)||value==='admin'||value==='records'||value.startsWith('tasks?')||value.startsWith('task/')?value:'home';};
export default function App(){return <NotificationProvider><ConfirmationProvider><AuthGate><StudioApp/></AuthGate></ConfirmationProvider></NotificationProvider>;}
function StudioApp(){
  const {user}=useAuth();
  ...
  return <div id="science-fieldnotes" className={'creation-platform'+(page.startsWith('task/')?' task-detail-page':'')}><a className="skip-link" href="#main">跳到主要内容</a>
    {!page.startsWith('task/')&&<header className="fn-header"><button className="fn-brand" onClick={()=>navigate('home')} aria-label="知序，回到首页"><span className="fn-brand-mark"><Bird/></span><span><strong>知序 · 创作空间</strong></span></button><nav className="fn-navigation" aria-label="主导航">{[...pages,...(user.role==='admin'?[{id:'admin',name:'管理中心'}]:[])].map(p=><button key={p.id} className="fn-nav" aria-current={(page===p.id||(p.id==='tasks'&&(page.startsWith('task')||page==='records')))?'page':undefined} onClick={()=>navigate(p.id)}>{p.name}</button>)}</nav><button className="ss-btn ss-primary" onClick={newTask}><Plus/>新建任务</button><AccountButton/></header>}
    ...
    </>}</main><footer className="ss-footer"><span>知序 · 自媒体创作平台</span><span>专属创作空间 · 北京时间 · 定时执行需保持服务运行</span></footer>
...
function Home({tasks,onOpen,onNew,onAll,onDeleted,onNavigate}:{tasks:CreationTask[];onOpen:(id:string,runId?:string)=>void;onNew:()=>void;onAll:()=>void;onNavigate:(page:string)=>void;onDeleted:(id:string)=>void}){
  ...
  return <><section className="studio-welcome"><div><h1>让每一个想法，<br/>有自己的创作空间。</h1><p>写一篇文章，制作一支短片，或者一组图文。<br/>亲手打磨，也可以设定时间，让灵感持续生长。</p><button className="ss-btn ss-primary" onClick={onNew}>开始新的创作 <ArrowRight/></button></div><div className="studio-collage" aria-hidden="true"><div className="collage-card collage-video"><Clapperboard/><div className="mini-landscape"><i/><b/></div></div><div className="collage-card collage-article"><FileText/><strong>一个好故事，<br/>从这里开始。</strong><i/><i/><i/></div><div className="collage-card collage-image"><Image/><b>记录<br/>每一份灵感</b></div></div></section>
...
function TaskList({tasks,onOpen,onNew,onDeleted,onRefresh,initialExecution}:{tasks:CreationTask[];onOpen:(id:string,runId?:string)=>void;onNew:()=>void;onDeleted:(id:string)=>void;onRefresh:()=>Promise<unknown>;initialExecution:string}){
  ...
  return <section><div className="platform-heading"><div><h1>{trash?'任务回收站':'任务列表'}</h1><p>{trash?'删除的任务保留作品与记录，恢复后先放入已归档，定时执行保持暂停。':'每个任务都有独立配置，点击卡片上的创作记录，可以打开之前的作品。'}</p></div><span className="ss-badge">{list.length} 个任务</span></div>
