...
export default function CreationRecords({onOpen}:{onOpen:(taskId:string,runId?:string)=>void}){
  ...
  return <section><div className="platform-heading"><div><span className="fn-label">CREATION HISTORY</span><h1>创作记录</h1><p>每执行一次任务，就有一条记录。这里包含手动与定时执行的历史。</p></div><span className="ss-badge">{list.length} 条记录</span></div><div className="task-filters"><input aria-label="搜索创作记录" placeholder="按任务名称搜索…" value={query} onChange={e=>setQuery(e.target.value)}/><button className="ss-btn" onClick={()=>setRefresh(v=>v+1)}>刷新记录</button><a className="ss-btn" href="#tasks">返回任务列表</a></div>{loading?<p className="inline-hint">正在读取创作记录…</p>:failed?<p className="inline-hint">读取失败，请点击刷新记录重试。</p>:<RecordList records={list} onOpen={run=>onOpen(run.task_id,run.id)}/>}</section>;
}
