...
def get(ident):
    with db.connect() as c:row=c.execute('SELECT * FROM image_jobs WHERE id=?',(ident,)).fetchone()
...
def create(settings, brief, sources, *, ai=True, ident=None):
    if ident:
        with db.connect() as c:old=c.execute('SELECT id FROM image_jobs WHERE id=?',(ident,)).fetchone()
        if old:
            render(ident)
            return get(ident)
    ...
    with db.connect() as c:
        c.execute('INSERT INTO image_jobs VALUES (?,?,?,?,?,?,?,?,?,?)',(ident,1,'draft',doc.model_dump_json(),settings.model_dump_json(),db.dump(sources),None,None,now,now))
    render(ident)
    return get(ident)
...
def render(ident):
    with db.connect() as c:
        claimed=c.execute("UPDATE image_jobs SET status='running',error=NULL WHERE id=? AND status='draft'",(ident,)).rowcount
    ...
    try:
        ...
        directory=config.DATA/'images'/ident/f'v{value["version"]}';directory.mkdir(parents=True,exist_ok=True)
        backdrop=None
        if settings.asset_id:
            with db.connect() as c:asset=c.execute('SELECT * FROM assets WHERE id=?',(settings.asset_id,)).fetchone()
        ...
        for i,card in enumerate(doc.cards):
            ...
            path=directory/f'card-{i+1:02d}.png';canvas.save(path);paths.append(path.relative_to(config.DATA).as_posix())
        with db.connect() as c:c.execute("UPDATE image_jobs SET status='needs_review',files=?,updated_at=? WHERE id=?",(db.dump(paths),db.now(),ident))
    except Exception as error:
        message=str(error) if isinstance(error,ValueError) else '图片生成失败，请检查素材文件后重试。'
        with db.connect() as c:c.execute("UPDATE image_jobs SET status='failed',error=?,updated_at=? WHERE id=?",(message,db.now(),ident))


def edit(ident,body):
    with db.connect() as c:
        c.execute('BEGIN IMMEDIATE');old=c.execute('SELECT version,status FROM image_jobs WHERE id=?',(ident,)).fetchone()
        ...
        if body.settings.asset_id:
            asset=c.execute('SELECT media_type FROM assets WHERE id=?',(body.settings.asset_id,)).fetchone()
            if not asset or not asset['media_type'].startswith('image/'):raise ValueError('请选择已有图片素材。')
        c.execute("UPDATE image_jobs SET document=?,settings=?,version=version+1,status='draft',files=NULL,error=NULL,updated_at=? WHERE id=?",
...
def bundle(ident):
    ...
    with zipfile.ZipFile(stream,'w',zipfile.ZIP_DEFLATED) as archive:
        for index,name in enumerate(value['files']):archive.write(config.DATA/name,f'card-{index+1:02d}.png')
        ...
        if value['settings'].get('asset_id'):
            with db.connect() as c:asset=dict(c.execute('SELECT * FROM assets WHERE id=?',(value['settings']['asset_id'],)).fetchone())
