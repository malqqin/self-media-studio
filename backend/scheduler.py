...
def tick():
    ...
    with db.connect() as c:
        claimed=c.execute('INSERT INTO daily_runs(day,status,message,at) VALUES (%s,%s,%s,%s) ON CONFLICT DO NOTHING',
    ...
    try:
        ...
        with db.connect() as c:
            candidates=c.execute('SELECT t.id,t.category FROM topics t WHERE t.kind=%s AND NOT EXISTS (SELECT 1 FROM jobs j WHERE j.topic_id=t.id) ORDER BY t.published_at DESC,t.score DESC,t.discovered_at DESC',
                                 ('live',)).fetchall()
    ...
    with db.connect() as c:
        c.execute('UPDATE daily_runs SET status=%s,message=%s,job_id=%s,at=%s WHERE day=%s',
